...
Also, Smart Rollups are not constrained to the layer 1 block baking rate and therefore can run far more transactions per second than layer 1.
...
When there is only one commitment left, either because all nodes published identical commitments or the refutation game eliminated all incorrect commitments, this correct commitment is said to be _cemented_ and becomes final and unchangeable.
A commitment can't be cemented all previous commitments are cemented.
