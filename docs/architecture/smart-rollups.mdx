...
Because Smart Rollup nodes behave in a deterministic manner, their states should all be the same if they have processed the same Smart Rollup inbox messages with the same kernel starting from the same origination level.
This state is referred to as the "state of the rollup."

Each rollup publishes a hash of its state, known as its _commitment_, on predefined intervals:

- The _commitment period_ runs for 60 consecutive layer 1 blocks.
During the commitment period, the rollups receive messages from the Smart Rollup inbox, process them, and update their internal states.
  ...
  At the end of the commitment period, each node generates a commitment and publishes it to layer 1.
...
During the refutation period, if the commitments are not all identical, the nodes play a _refutation game_ to identify the correct commitment.

  When there is only one commitment left, either because all nodes published identical commitments or the refutation game eliminated all incorrect commitments, this correct commitment is said to be _cemented_ and therefore final and unchangeable.
...
1. At the same time as the new commitment period begins, a refutation period for the previous commitment period begins, during which time other nodes can publish a concurrent commitment to refute a published commitment.
...
At the end of the commitment period, the rollup node must publish its state to layer 1, which is called its _commitment_.
...
This commitment asserts that the rollup node has responded to every message in the rollup inbox during in the commitment period and updated its state according to the kernel code of the Smart Rollup.
...
The protocol eliminates the incorrect commitment because it has no stake.
