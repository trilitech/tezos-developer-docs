...
1. When the certificate is confirmed in a block, the DAL splits the data into shards and layer 1 assigns those shards to bakers.
...
1. DAL nodes share the data through the peer-to-peer network.
...
1. The Smart Rollup node uses or stores the data.
