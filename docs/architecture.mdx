...
There are several Tezos blockchains.
...
Its nodes can update the protocol that controls the possible operations and how they are processed via an online governance process.
...
Updates to the protocol can change this logic through a voting process.
...
The node and the baker can be on different computers, which allows the node to manage communication and shield bakers from network attacks.
...
- Getting contract balances and data
