...
:::note Planning ahead
Before trying to run the code yourself, look at [Explorus](https://beta.explorus.io/dal) and choose a slot that is not currently being used.
:::
...
1. Using the values of the commitment and proof from the previous command, post the certificate to layer 1 with this command:
   ...
   If you don't see information about the attested slot, it's possible that the attesters for the network are offline.
