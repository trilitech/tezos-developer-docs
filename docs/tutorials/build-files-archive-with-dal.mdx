...
1. The Smart Rollup node uses and stores the data.
Smart Rollups must store the data if it needs it because it is available on the DAL for only a short time.
