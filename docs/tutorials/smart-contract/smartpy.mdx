...
1. Go to the the `Test_scenario` folder:

   ```bash
   cd Test_scenario
   ```

1. Run the following command to originate the smart contract, changing `$MY_TZ_ADDRESS` to the name of the wallet that you created earlier in the tutorial:
...
You can also call entrypoints from block explorers:
