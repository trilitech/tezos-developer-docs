...
4. **Offer the option to select the RPC node**: the network traffic on Tezos can get busy, but you can provide a better user experience (UX) by adding an option to switch to a different Tezos node if the default one becomes too slow. Some power users may also like having the choice to enter the address of their favourite node!
