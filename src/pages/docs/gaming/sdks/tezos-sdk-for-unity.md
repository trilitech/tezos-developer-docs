...
1. **Connect to a Tezos wallet**: The wallet-pairing feature enables users to authenticate using their Tezos blockchain credentials, sign transactions, access on-chain assets, and utilize cryptocurrency for purchasing services or in-game assets.
2. **Access blockchain data**: Developers can use the SDK to retrieve data from the Tezos blockchain, such as in-game asset ownership. Users can manage asset storage through smart contracts or off-chain views, granting access to data stored on the blockchain.
3. **Invoke smart contracts**: Games can generate calls to Tezos smart contracts, which users can sign, unlocking the potential of Tezos smart contracts. This functionality supports transactions like purchasing additional features or creating in-game assets based on achievements.
4. **Establish true ownership of in-game assets**: The SDK allows users to own and verify the authenticity of in-game assets via the Tezos blockchain, using smart contracts to confirm data validity.

The Tezos SDK is compatible with Desktop, Android, iOS, and browser platforms, and serves as a valuable resource for developing Tezos decentralized applications (dApps).

## API Summary

The SDK's API includes seven methods, which, together with the BeaconSDK and Netezos libraries, provide comprehensive access to the Tezos Blockchain for your games:
...
If you're new to Unity, download the Unity editor for free at https://unity.com/download. The free "personal" Unity editor offers a comprehensive core feature-set for creating 2D and 3D games, apps, and environments, including AR, VR, and Web3 development.
...
### Read detailed [documentation.](https://opentezos.com/gaming/unity-sdk/) on OpenTezos.