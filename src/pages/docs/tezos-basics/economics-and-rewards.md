...
For more detail about the Tezos baking process, please refer to the ["*Baking*"](/baking) module.

### Inflation

It's important to note that the Tez token is based on a model with inflation. By adding up the baking and endorsing block rewards, each new high-priority block generates 40 new Tez. A new block is created roughly every 30 seconds. This represents 42 Million Tez per year ($\approx$ 80 ꜩ $\times$ 60 mins $\times$ 24 hours $\times$ 365 days). At the Tezos launch, the network was composed of 763 Million Tez.
