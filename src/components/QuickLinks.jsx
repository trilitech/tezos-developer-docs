...
export function QuickLinks({ children }) {
  return (
    <div className="not-prose my-12 grid grid-cols-1 sm:grid-rows-6 sm:grid-flow-col sm:grid-cols-3 sm:gap-1">
        <h2 className="font-semibold text-lg text-slate-800 uppercase">Tezos Basics</h2>
        {children.slice(0, Math.ceil(children.length / 3))}

        <h2 className="font-semibold text-lg text-slate-800 uppercase">NFTs, Gaming and DeFi</h2>
        {children.slice(Math.ceil(children.length / 3), Math.ceil(children.length / 3) * 2)}
 
        <h2 className="font-semibold text-lg text-slate-800 uppercase">DApp Development</h2>
    ...
    <div className="relative flex items-center h-20 pr-4">
      ...
      <div className="relative overflow-hidden rounded-xl p-2 flex justify-center h-full items-center">
        ...
        <div className="flex-grow">
          <h2 className="mt-4 font-display font-bold text-sm text-slate-700 dark:text-white" style={{ lineHeight: '1.5rem' }}>
            {href === '' ? (
              <>
                {title}
                <span className="ml-2 block sm:inline-flex items-center rounded-full bg-green-50 px-2 py-1 text-xxs font-medium text-green-700 ring-1 ring-inset ring-green-600/10">Coming Soon</span>
              </>
            ) : (
              <Link href={href}>
